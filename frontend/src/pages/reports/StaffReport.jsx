...
function StaffReport() {
    ...
    const staffRows = useMemo(() => {
        ...
        return raw.map((member) => ({
            ...
            name: `${member.first_name || ""} ${member.last_name || ""}`.trim() || "Unnamed",
    ...
    const roleData = useMemo(() => {
        ...
        staffRows.forEach((s) => {
            const key = s.role || "—";
            map[key] = (map[key] || 0) + s.appointments;
        });
    ...
    const statCards = useMemo(() => {
        const totalAppointments = staffRows.reduce((sum, row) => sum + row.appointments, 0);
        const totalCompleted = staffRows.reduce((sum, row) => sum + row.completed, 0);
        const totalPending = staffRows.reduce((sum, row) => sum + row.pending, 0);
            ...
            <div className="flex min-w-0 flex-1 flex-col">
                ...
                <main className="flex-1 bg-beige px-4 py-5 sm:px-6 md:px-8 md:py-6">
                    <div className="flex flex-wrap items-center gap-2 mb-5">
                        <Link to="/company/reports" className="text-xs font-bold uppercase tracking-wide text-slate hover:text-navy transition">
                            Reports &amp; Analytics
                        </Link>
                    ...
                    <div className="flex flex-col gap-5 mb-6 lg:flex-row lg:items-start lg:justify-between">
                        <div>
                            <h1 className="font-serif text-2xl text-navy sm:text-3xl md:text-4xl">Staff Report</h1>
                            <p className="text-sm text-slate mt-1.5 max-w-[520px]">
                                Review staff performance, working hours, appointments, and revenue generated.
                            </p>
                        </div>

                        <div className="flex flex-col gap-3 sm:flex-row sm:items-center lg:flex-col lg:items-end">
                            <button type="button" className="bg-white border border-gray/30 rounded-lg px-4 py-2.5 flex items-center justify-center gap-2 text-sm font-bold text-navy hover:border-navy transition">
                    ...
                    <div className="mb-6 grid grid-cols-1 gap-6 lg:grid-cols-2">
                        <div className="rounded-xl border border-gray/20 bg-white p-5 shadow-sm sm:p-6">
                            <h2 className="font-serif text-lg font-bold text-navy mb-1">Staff Trend</h2>
                            <p className="text-xs text-slate mb-5">Appointments vs completed</p>

                            <div className="h-[240px] w-full">
                                {staffTrendData.length === 0 ? (
                                    <div className="flex h-full items-center justify-center">
                                        <p className="text-sm text-slate">No staff data available.</p>
                                    </div>
                                ) : (
                                    <ResponsiveContainer width="100%" height="100%">
                                        <AreaChart data={staffTrendData} margin={{ top: 10, right: 10, left: -20, bottom: 0 }}>
                        ...
                        <div className="rounded-xl border border-gray/20 bg-white p-5 shadow-sm sm:p-6">
                            <h2 className="font-serif text-lg font-bold text-navy mb-1">Role Breakdown</h2>
                            <p className="text-xs text-slate mb-5">Appointments by staff role</p>

                            <div className="h-[240px] w-full">
                                {roleData.length === 0 ? (
                                    <div className="flex h-full items-center justify-center">
                                        <p className="text-sm text-slate">No role data available.</p>
                                    </div>
                                ) : (
                                    <ResponsiveContainer width="100%" height="100%">
                                        <BarChart data={roleData} margin={{ top: 10, right: 10, left: -20, bottom: 0 }}>
                                            ...
                                            <Bar dataKey="count" radius={[6, 6, 0, 0]}>
                                                {roleData.map((entry) => (
                                                    <Cell key={entry.role} fill={roleColors[entry.role] || "#000C1E"} />
                                                ))}
                                            </Bar>
                                        </BarChart>
                                    </ResponsiveContainer>
                    ...
                    <div className="bg-white rounded-xl border border-gray/20 shadow-sm overflow-hidden">
                        <div className="p-5 sm:p-6 border-b border-gray/20">
                            <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
                                <div>
                                    <h2 className="font-serif text-lg text-navy sm:text-xl">Staff Performance</h2>
                                    <p className="text-xs text-slate mt-1">Detailed performance breakdown for each staff member.</p>
                                </div>

                                <select
                                    value={staffFilter}
                                    onChange={(event) => { setStaffFilter(event.target.value); setCurrentPage(1); }}
                                        ...
                                        filteredStaff.map((member) => (
                                            <tr key={member.id} className="hover:bg-beige/30 transition">
                                                <td className="px-6 py-4">
                                                    ...
                                                    <p className="text-xs text-slate mt-0.5">{member.role}</p>
                                                    <p className="text-[11px] text-gray mt-0.5">{member.staffId}</p>
                                                </td>

                                                <td className="px-6 py-4 text-sm font-bold text-navy">{member.appointments}</td>
                                                ...
                                                <td className="px-6 py-4 text-sm font-bold text-amber-700">{member.rejected}</td>
                                                ...
                                                <td className="px-6 py-4 text-sm font-bold text-slate">{member.pending}</td>
                                            </tr>
                                        ))
                                    ) : (
                                        <tr>
                                            <td colSpan="7" className="px-6 py-12 text-center">
                                                <p className="text-sm font-bold text-navy">No staff data found.</p>
                                                <p className="text-xs text-slate mt-1">Try changing the selected staff member.</p>
                                            </td>
                                        </tr>
                                    )}
                                </tbody>
